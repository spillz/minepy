            ...
            if self.update_ref_pos != new:
                ...
                for s in list(self.sectors):
                    if abs(new[0] - s[0])>LOADED_SECTORS*SECTOR_SIZE or abs(new[2] - s[2]) > LOADED_SECTORS*SECTOR_SIZE:
                        ...
                        vt = None
                        if self.sectors[s].vt != None:
                            vt = self.sectors[s].vt
#                            self.sectors[s].vt.delete()
                        del self.sectors[s].vt
                        del self.sectors[s].vt_data
                        del self.sectors[s].batch
                        del self.sectors[s].model
                        del self.sectors[s].group
                        del self.sectors[s]
                        if vt:
                            vt.delete()
#                        print(gc.collect(1))
