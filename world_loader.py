    ...
    def _loader(self):
        ...
        while True:
            ...
            if msg == 'sector_blocks':
                ...
                self._pipe.send_bytes(cPickle.dumps(['sector_blocks',[pos, self.blocks, self.vt_data]],-1))
            if msg == 'set_block':
                spos = data[0]
                pos = data[1]
                block_id = data[2]
                ...
                self._pipe.send_bytes(cPickle.dumps(['sector_blocks',[spos, self.blocks, self.vt_data]],-1))
...
class Sector(object):
    def __init__(self, position, world, sector_block_delta = None):
        ...
        self.world = world
        self.sector_block_delta = sector_block_delta
        self.blocks = numpy.zeros((SECTOR_SIZE+2,SECTOR_HEIGHT,SECTOR_SIZE+2),dtype='u2')
    ...
    def _initialize(self):
        ...
        if self.sector_block_delta:
            print(list(self.sector_block_delta))
            print(list(self.sector_block_delta.itervalues()))
            for p in self.sector_block_delta:
                self.blocks[p] = self.sector_block_delta[p]
...
#            self.sectors[s].calc_vertex_data()
    ...
    def get_sector_data(self, spos, sector_block_delta = None):
        spos = sectorize(spos)
        if spos in self.sectors and sector_block_delta == None:
        ...
        if spos in self.sectors:
            del self.sectors[spos]
        t0=time.time()
        s = Sector(spos, self, sector_block_delta)
        t1 = time.time()
        s._initialize()
        t2 = time.time()
